...
function ExerciseCard({ exercise, value, onChange, onStartRest }) {
  return (
    <div className="exercise-card">
      <div className="exercise-head">
        <div>
          <h3>{exercise.name}</h3>
          <p>{exercise.sets} séries • {exercise.reps} reps • descanso {exercise.rest}s</p>
        </div>
        <button className="ghost-button" onClick={() => onStartRest(exercise.rest)}>
          Descanso
        </button>
      </div>
      ...
      <div className="exercise-grid">
        ...
        <label>
          ...
          <input
            ...
            onChange={(e) => onChange({ ...value, doneReps: e.target.value })}
            placeholder="Ex: 10"
          />
        </label>
      </div>
